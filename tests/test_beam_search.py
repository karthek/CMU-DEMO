...
class Scores:
    ...
    def evaluate(self, context, candidate):
        ...
        value = self.values.get(time, 0)
        return ScoreEvaluation(value, {"components": [{"code": "TEST_SCORE", "value": value}]}, [])


class BeamSearchTests(unittest.TestCase):
    def setUp(self):
        self.context = create_simulated_coordinator().get_trip_context("DL1425", "2026-09-11")

    def test_best_parent_survives_worse_children(self):
        seed = plan("16:00")
        for depth in (1, 2, 3, 5):
            result = BeamSearchPlanner(1, depth, Scores({"16:00": 100})).search(self.context, [seed])
    ...
    def test_better_child_replaces_parent(self):
        result = BeamSearchPlanner(1, 2, Scores({"16:00": 5, "16:10": 10})).search(self.context, [plan("16:00")])
    ...
    def test_equivalent_timestamps_deduplicate_before_scoring(self):
        ...
        result = BeamSearchPlanner(2, 1, critic).search(self.context, [plan("16:00", "First"), plan("16:00:00", "Second")])
    ...
    def test_finalists_distinct_and_width_enforced(self):
        for width in (1, 2, 4):
            result = BeamSearchPlanner(width, 4, Scores({})).search(self.context, [plan("16:00"), plan("16:10"), plan("16:00")])
    ...
    def test_ties_preserve_input_order_and_parents(self):
        ...
        result = BeamSearchPlanner(2, 3, Scores({})).search(self.context, seeds)
    ...
    def test_duplicate_children_keep_first_real_path(self):
        ...
        result = BeamSearchPlanner(2, 2, Scores({"16:10": 100})).search(
            self.context, [plan("16:00", "First"), plan("16:20", "Second")])
    ...
    def test_repeated_runs_identical_and_inputs_unchanged(self):
        ...
        first = planner.search(self.context, seeds)
        for _ in range(4):
            self.assertEqual(planner.search(self.context, seeds), first)
    ...
    def test_depth_counts_initial_ranking_plus_refinement_rounds(self):
        for depth, expected in [(1, "16:00"), (2, "16:10"), (3, "16:20")]:
            ...
            result = BeamSearchPlanner(1, depth, scores).search(self.context, [plan("16:00")])
    ...
    def test_empty_candidates_rejected(self):
        with self.assertRaisesRegex(ValueError, "initial_candidates"):
            BeamSearchPlanner().search(self.context, [])
