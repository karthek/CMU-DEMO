...
class V8MCPTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_exact_v7_schemas_unchanged(self):
        reference = json.loads(
            (ROOT / "tests/fixtures/v7/mcp_schemas.json").read_text(encoding="utf-8")
        )
        self.assertEqual(len(reference["tools"]), 2)
        for before, after in zip(reference["tools"], TOOLS[:2], strict=True):
